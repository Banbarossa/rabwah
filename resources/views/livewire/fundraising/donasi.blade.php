<div>
    <section class="bg-gradient-to-b from-brand-cream to-white py-16 hidden lg:block">
        <x-section-title
            ...
            <div class="grid md:grid-cols-3 gap-8 mt-18">
                ...
                @foreach ($this->campaignPrioritas as $item)
                    <div class="bg-white shadow-sm hover:shadow-lg rounded-2xl overflow-hidden transition">
                        <img src="{{ $item->thumbnail }}" class="aspect-[16/10] w-full object-cover"
                             alt="{{ $item->title }}">
                        <div class="p-5">
                            ...
                            <a href="{{route('donasi.detail',['slug' => $item->slug])}}"
                               class="inline-block bg-brand-green/90 text-white text-sm px-4 py-2 rounded-md hover:bg-brand-green">
                                    Selengkapnya
                            </a>
        ...
        <div class="max-w-6xl mx-auto px-4">
            <div class="text-center mb-10">
                ...
                <p class="text-gray-600 max-w-2xl mx-auto">
                    Selain tiga program utama, pesantren juga membuka donasi untuk kebutuhan insidental seperti
                    pembangunan sumur bor, dapur umum, dan bantuan kesehatan santri.
                </p>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-8">
                @foreach($this->campaignSosial as $item)
                    <div class="bg-white shadow-sm rounded-2xl overflow-hidden hover:shadow-lg transition">
                        <img src="{{ $item->thumbnail}}" class="aspect-[16/9] w-full object-cover"
                             alt="{{ $item->title }}">
                        <div class="p-5">
                            <h3 class="text-lg font-semibold text-brand-green mb-2">{{ $item->title }}</h3>
                            <p class="text-gray-600 text-sm mb-4 line-clamp-3">{{ $item->excerpt }}</p>
                            ...
                            <div class="mb-3">
                                <div class="flex justify-between text-sm text-gray-500">
                                    <span>Terkumpul: {{ format_rupiah($item->capaian) }}</span>
                                    <span>Target: {{ format_rupiah($item->target_amount) }}</span>
                                </div>
                                <div class="w-full bg-gray-200 rounded-full h-2 mt-1">
                                    <div class="bg-brand-gold h-2 rounded-full" style="width: {{$item->persentase}}%;"></div>
                                </div>
                            </div>
