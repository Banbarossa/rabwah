<div>
    <div class="flex">
        ...
        <flux:spacer/>
        <flux:button variant="primary"  wire:navigate href="{{route('post.tag.form')}}">Tambah
        </flux:button>
    </div>
    <div class="mt-8">
        <x-table.container>
                ...
                @forelse($tags as $index => $tag)
                    <x-table.row variant="hovered">
                        ...
                        <x-table.cell align="right">
                            <flux:button  icon="eye" size="sm" :href="route('post.tag.form',['category'=>$tag->id])" wire:navigate></flux:button>
                            <flux:button  icon="trash" size="sm" variant="danger" wire:click="confirmDestroy({{$tag->id}})"></flux:button>
                        </x-table.cell>
                    </x-table.row>
                @empty
                    <x-table.row>
